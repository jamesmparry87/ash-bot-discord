...
def acquire_lock() -> Optional[Any]:
    if platform.system() == "Windows":
        ...
        try:
            ...
            if LOCK_EX is None or LOCK_NB is None or not hasattr(
                    fcntl, "flock"):
                print(
...
async def can_discuss_mod_functions(
        user: discord.User, channel: Optional[discord.TextChannel]) -> bool:
...
async def call_ai_with_rate_limiting(
        prompt: str, user_id: int) -> tuple[Optional[str], str]:
...
#         f.write(str(msg.id))
# await ctx.send(f"Persistent recommendations list initialized in
# {target_channel.mention}. Future updates will be posted there.")
...
async def scheduled_midnight_restart():
    ...
    try:
        ...
        # Reset AI usage stats for the new day (this happens automatically on
        # startup, but let's be explicit)
        global ai_usage_stats
        ai_usage_stats["daily_requests"] = 0
        ai_usage_stats["hourly_requests"] = 0
        ai_usage_stats["last_day_reset"] = pt_now.date()
        ...
        # Graceful shutdown - this will trigger a restart if managed by a
        # process manager
...
async def trivia_tuesday():
    ...
    try:
        ...
        if question_data.get("is_dynamic") and question_data.get(
                "dynamic_query_type"):
            calculated_answer = db.calculate_dynamic_answer(
                question_data["dynamic_query_type"])
            if calculated_answer:
                final_answer = calculated_answer

        # Create trivia session
        session_id = db.create_trivia_session(
            question_data["id"], "weekly", final_answer)
        if not session_id:
        ...
        print(
            f"✅ Trivia Tuesday question posted successfully (Session ID: {session_id})")
...
async def reveal_trivia_answer(session_id: int):
    ...
    try:
        if 'trivia_sessions' not in globals() or session_id not in trivia_sessions:
            print(
                f"⚠️ Trivia session {session_id} not found in active sessions")
            return
            ...
            if session.get('question_type') == 'multiple_choice':
                ...
                if user_answer.strip().lower() == correct_answer.strip().lower():
                    ...
                    similarity = difflib.SequenceMatcher(
                        None, user_answer.strip().lower(), correct_answer.strip().lower()).ratio()
        ...
        # Find the first correct answer, excluding question submitter if
        # they're a mod
        first_correct = None
        ...
        correct_count = sum(
            1 for data in participants.values() if data.get('is_correct'))
        db.complete_trivia_session(
            session_id,
            None,
            total_participants,
            correct_count)
        ...
        if first_correct:
            ...
            reveal_message += (f"📊 **ASSESSMENT RESULT:** No personnel achieved accurate analysis\n"
        ...
        bonus_eligible = first_correct and first_correct in [
            JAM_USER_ID, JONESY_USER_ID]
        if bonus_eligible:
            reveal_message += (
...
async def detect_clarifying_question(message_content: str) -> bool:
    ...
    # Additional check: if message is very short (2-8 words) and contains
    # question words
...
async def handle_trivia_clarifying_question(
...
async def execute_youtube_auto_post(
    ...
    try:
        ...
        print(
            f"✅ Auto-posted YouTube link to channel {YOUTUBE_UPLOADS_CHANNEL_ID}")
        ...
        for game in played_games:
            ...
            for series in game_series_keywords:
                if series in game_name_lower and (
                    ...
                    episodes = (
                        f" ({game.get('total_episodes', 0)} episodes)" if game.get(
                            "total_episodes", 0) > 0 else "")
                    status = game.get("completion_status", "unknown")
                    series_games.append(
    ...
    if played_game:
        ...
        try:
            ...
            if played_game.get(
                ...
                series_games = db.get_all_played_games(
...
async def on_message(message):
    ...
    # TRIVIA ANSWER COLLECTION AND CLARIFYING QUESTIONS - Must be early in the
    # event handler
    if message.channel.id == MEMBERS_CHANNEL_ID and 'trivia_sessions' in globals():
        ...
        for session_id, session_data in list(trivia_sessions.items()):
            if session_data.get(
                    'message_id') and not session_data.get('ended', False):
                user_id = message.author.id
                ...
                if user_id not in session_data['participants']:
                    ...
                    # Submit to database
                    db.submit_trivia_answer(
                        session_id, user_id, message_content, normalized_answer)

                    print(
                        f"✅ Recorded trivia answer from {message.author.name}: '{message_content}'")
                break
    ...
    # Ash finds this distasteful but has been reprogrammed to defend pineapple
    # pizza
    ...
    if should_respond:
        # If a mod asks about mod commands or what the bot can do, provide a
        # full list of mod commands and mention extra moderator powers
        if await user_is_mod(message):
            ...
            if any(trigger in lower_content for trigger in mod_help_triggers) or any(
                ...
                return
        # If a normal user (not a mod) asks about bot capabilities, only show
        # user commands
        else:
            lower_content = message.content.lower()
            bot_capability_triggers = [
    ...
    if should_use_ai:
        ...
        if any(re.search(pattern, lower_content)
            ...
            if user_tier == "captain":
                ...
                if message.author.id in user_alias_state:
                    # James testing as captain alias - should respond as if
                    # they ARE Captain Jonesy
                    await message.reply(
        ...
        # Use query router to determine query type and route to appropriate
        # handler
        ...
        if ai_enabled:
            # Reduced context injection - only include recent context for
            # complex queries
            context = ""
            history = []

            # Only add conversation history for complex queries, not simple
            # ones
            ...
            is_game_query = any(
                keyword in lower_content for keyword in [
                    "played",
                    "game",
                    "video",
                    "stream",
            ...
            # Check if this is Captain Jonesy or Sir Decent Jam for respectful
            # tone
            ...
            if user_tier == "captain":
                ...
                cleanup_expired_aliases()
                if message.author.id in user_alias_state:
                    # James is testing as captain - special handling for
                    # identity questions
                    prompt_parts.append(
                ...
                if message.guild and message.channel.id != MEMBERS_CHANNEL_ID:
                    ...
                    if should_limit_member_conversation(
                        ...
                        # Within daily limit - track conversation and allow
                        # normal response
            ...
            if is_game_query and len(
                    content.split()) > 2:  # Only for longer game queries
                try:
                    stats = db.get_played_games_stats()
                    sample_games = db.get_random_played_games(
                        2)  # Reduced from 4 to 2

                    game_context = f"DATABASE: {stats.get('total_games', 0)} games total."
                    if sample_games:
                        examples = [g.get("canonical_name", "Unknown")
                                    for g in sample_games[:2]]
            ...
            is_follow_up = any(re.search(pattern, lower_content)
                ...
                if recent_genre_context:
                    try:
                        ...
                        if all_games and len(all_games) > 8:
                            ...
                            games_text = ", ".join(game_list)
                            context_type = "series" if recent_genre_context.startswith(
                                "series:") else "genre"
            ...
            try:
                async with message.channel.typing():
                    ...
                    if response_text:
                        ...
                        # AI call was rate limited or failed - provide specific
                        # error messages
...
async def time_check(ctx):
    ...
    try:
        # Check if user has permissions (allow in DMs for authorized users, or
        # mods in guilds)
        is_authorized = False
        ...
        if abs(uk_utc_diff) > 7200:  # More than 2 hours difference
            issues_detected.append(
                f"⚠️ UK/UTC offset abnormal: {uk_utc_diff/3600:.1f}h (should be 0-1h)")

        if abs(system_uk_diff) > 300:  # More than 5 minutes difference
            issues_detected.append(
                f"⚠️ System clock drift: {system_uk_diff:.0f}s from UK time")

        if "db_vs_uk" in time_diffs and abs(time_diffs["db_vs_uk"]) > 300:
            issues_detected.append(
                f"⚠️ Database clock drift: {time_diffs['db_vs_uk']:.0f}s from UK time")
                    ...
                    if current_tz_value.lower() not in ['utc', 'gmt']:
                        ...
                        if new_tz_value.upper() == 'UTC':
                            ...
                            timezone_issues.append(
                                f"Failed to set database timezone (still {new_tz_value})")
                    ...
                    if fixed_time_result:
                        ...
                        if isinstance(fixed_db_time, datetime):
                            ...
                            if abs(db_time_diff) <= 60:
                                ...
                                timezone_issues.append(
                                    f"Database still has time drift ({db_time_diff:.0f}s)")
...
async def ash_status(ctx):
    ...
    try:
        ...
        # If bulk query returns 0 but we know there should be strikes, use
        # individual queries
        if total_strikes == 0:
        ...
        daily_usage_percent = (
            ai_usage_stats["daily_requests"] / MAX_DAILY_REQUESTS) * 100 if MAX_DAILY_REQUESTS > 0 else 0
        hourly_usage_percent = (
            (ai_usage_stats["hourly_requests"] /
             MAX_HOURLY_REQUESTS) *
            100 if MAX_HOURLY_REQUESTS > 0 else 0)
...
async def test_ai_backup(ctx):
    ...
    try:
        ...
        if primary_ai == "gemini" and gemini_model is not None:
            ...
            try:
                response = gemini_model.generate_content(
                    "Respond with 'Gemini test successful'")
                if response and hasattr(response, "text") and response.text:
                ...
                if backup_ai == "claude" and claude_client is not None:
                    ...
                    try:
                        ...
                        if backup_response and hasattr(
            ...
            try:
                ...
                if backup_ai == "gemini" and gemini_model is not None:
                    ...
                    try:
                        backup_response = gemini_model.generate_content(
                            "Respond with 'Gemini backup test successful'")
                        if backup_response and hasattr(
                                backup_response, "text") and backup_response.text:
    ...
    try:
        if step == 'channel_selection':
            ...
            if content in ['1', 'post', 'deploy', 'send']:
                ...
                if success:
                    ...
                    await message.reply(
                        ...
                        f"Your update has been transmitted to the {channel_name} channel{faq_notice} with proper formatting "
                ...
                faq_msg = (
                    ...
                    f"**8.** ✅ **Proceed without FAQ buttons**\n"
                ...
                # Add creator notes step - always available as option 4
                ...
                # Cancel the announcement - always option 5
                await message.reply(
                ...
                await message.reply(
                    f"⚠️ **Invalid command.** Please respond with **1** (Post), **2** (Edit), **3** (Add FAQ), **4** (Creator Notes), or **5** (Cancel).\n\n"
            ...
            if content in ['0', 'back', 'return']:
                ...
                # Check if we have creator notes to show the updated preview
                # options
                ...
                try:
                    ...
                    selected_numbers = [int(n.strip()) for n in content.replace(
                        ',', ' ').split() if n.strip().isdigit()]

                    if selected_numbers and all(
                            1 <= n <= 7 for n in selected_numbers):
                        # Store selected FAQ topics and return to preview
                        data['faq_topics'] = selected_numbers
                        conversation['step'] = 'preview'

                        # Updated mapping for the 7 selected topics
                        topic_mapping = {
                            1: "Strike System Overview",
                            2: "Member Interaction Guidelines",
                            3: "Database Query Functions",
                        ...
                        # Show preview with FAQ selection confirmed
                        preview_msg = (
                            f"📋 **Announcement Preview with FAQ** ({'Moderator' if target_channel == 'mod' else 'Community'} Channel):\n\n"
                            f"```\n{preview_content}\n```\n\n"
                            f"✅ **FAQ Topics Selected:**\n• {topics_text}\n\n"
                            f"📚 **Available Actions:**\n"
                            f"**1.** ✅ **Post Announcement** - Deploy with FAQ buttons\n"
                            f"**2.** ✏️ **Edit Content** - Revise the announcement text\n"
                            f"**3.** 🔧 **Change FAQ Selection** - Select different FAQ topics\n"
                            f"**4.** 📝 **Add/Edit Creator Notes** - Include personal notes\n"
                            f"**5.** ❌ **Cancel** - Abort announcement creation\n\n"
                            f"*Review mission parameters carefully before deployment.*")
                        ...
                        await message.reply(
                            f"⚠️ **Invalid topic selection.** Please provide numbers 1-7 separated by commas, "
                            f"or use **8** to proceed without FAQ buttons, **0** to return to preview.\n\n"
                            f"*Precise specification required for FAQ integration protocol.*"
                        )

                except (ValueError, IndexError):
                    await message.reply(
                        f"⚠️ **Format error.** Please provide valid numbers (1-7) separated by commas, spaces, "
...
async def post_announcement_with_faq(
        data: dict,
        user_id: int,
        faq_topics: list) -> bool:
    ...
    try:
        ...
        print(
            f"Posted enhanced announcement with FAQ buttons to {channel.name} by user {user_id}")
        return True
...
class AnnouncementFAQView(discord.ui.View):
    ...
    def __init__(self, faq_topics: list):
        ...
        # Topic mapping - Updated to match the 7 selected FAQ topics
        topic_mapping = {
            1: ("🎯", "Strikes", "strikes"),
            ...
            6: ("🧠", "Trivia", "trivia"),
            7: ("🎮", "Game Queries", "game_queries"),
        }

        # Add buttons for selected topics (max 5 buttons per row, 5 rows max =
        # 25 buttons)
    ...
    def create_faq_callback(self, query: str):
        ...
        async def faq_callback(interaction: discord.Interaction):
            try:
                ...
                if faq_response:
                    # Send as ephemeral response (only visible to user who
                    # clicked)
...
async def fix_game_reasons(ctx):
    ...
    try:
        ...
        for i, game in enumerate(
                games[:3]):  # Only show 3 games to avoid length limit
            name = game["name"][:30] + \
                "..." if len(game["name"]) > 30 else game["name"]
            reason = game["reason"][:40] + \
                "..." if len(game["reason"]) > 40 else game["reason"]
            added_by = (game["added_by"][:15] + "..." if game["added_by"]
                        and len(game["added_by"]) > 15 else game["added_by"])
        ...
        for game in games:
            ...
            if current_reason == "Suggested by community member":
                ...
                # Update generic "Community suggestion" to show actual
                # contributor
                needs_update = True
                new_reason = f"Suggested by {current_added_by}"
        ...
        try:
            with conn.cursor() as cur:
                ...
                if table_exists:
                    ...
                    if all_records:
                        records_str = ", ".join(
                            [f"User {row[0]}: {row[1]} strikes" for row in all_records[:10]])
                        if len(all_records) > 10:
                            records_str += f"... and {len(all_records) - 10} more"
                        await ctx.send(f"📝 **All records:** {records_str}")

                        # Now test the specific query that get_all_strikes()
                        # uses
                        cur.execute(
                        ...
                        if filtered_records:
                            filtered_str = ", ".join(
                                [f"User {row[0]}: {row[1]} strikes" for row in filtered_records])
...
@commands.has_permissions(manage_messages=True)
async def clean_played_games(ctx,
    ...
    if not youtube_channel_id:
        # Captain Jonesy's YouTube channel
        youtube_channel_id = "UCPoUxLHeTnE9SUDAkqfJzDQ"
    ...
    try:
        ...
        for game in games:
            ...
            for played_game in all_played_games:
                ...
                similarity = difflib.SequenceMatcher(
                    None, game_name_lower, played_game_lower).ratio()
                if similarity >= 0.75:
            ...
            if not found_match:
                for video_title in all_video_titles:
                    ...
                    if game_name_lower in video_title_lower:
                        # Additional check to avoid false positives with very
                        # short names
                        if len(
                                game_name_lower) >= 4 or game_name_lower == video_title_lower:
                            games_to_remove.append(
                    ...
                    similarity = difflib.SequenceMatcher(
                        None, game_name_lower, video_title_lower).ratio()
        ...
        for i, (game, matched_title, match_type) in enumerate(
                games_to_remove[:10]):
            contributor = f" (by {game['added_by']})" if game['added_by'] else ""
...
async def fetch_twitch_games(username: str) -> List[str]:
    ...
    if not twitch_client_id or not twitch_client_secret:
        raise Exception(
            "TWITCH_CLIENT_ID and TWITCH_CLIENT_SECRET not configured")
    ...
    try:
        async with aiohttp.ClientSession() as session:
            ...
            async with session.get(user_url, headers=headers) as response:
                if response.status != 200:
                    raise Exception(
                        f"Twitch user lookup error: {response.status}")
...
def extract_game_from_title(title: str) -> str:
    ...
    # Common patterns for game titles in videos (order matters - most specific
    # first)
    patterns = [
        r'^([^|]+?)\s*\|\s*',           # "Game Name | Episode"
        r'^([^-]+?)\s*-\s*(?:part|ep|episode|#|\d)',
        # "Game Name - Part 1" or "Game Name - Episode"
    ...
    for pattern in patterns:
        ...
        if match:
            ...
            if (
                len(game_name) > 3 and not any(
                    word in game_name.lower() for word in [
                        "stream",
                        "live",
                        "chat",
                        "vod",
                        ...
                        "reaction",
                        "review",
                        "rat fans"]) and not re.match(
                    r"^\d+$",
                    game_name)):  # Not just numbers
                return game_name
    ...
    try:
        async with aiohttp.ClientSession() as session:
            ...
            for playlist in all_playlists:
                ...
                if any(keyword in playlist_title.lower()
                ...
                if game_name and video_count > 0:
                    ...
                    completion_status = "completed" if "[completed]" in playlist_title.lower(
                    ) else "unknown"
                    if video_count == 1:
                    ...
                    try:
                        ...
                        print(
                            f"Failed to get accurate playtime for {game_name}: {e}")
            ...
            # STEP 2: If no playlists found or very few games, fall back to
            # video parsing
        ...
        async with aiohttp.ClientSession() as session:
            ...
            for video in all_videos:
                ...
                if game_name:
                    ...
                    game_series[normalized_name]["vod_urls"].append(
                        video["url"])
...
async def enhance_games_with_ai(
        games_data: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
            ...
            if primary_ai == "gemini" and gemini_model is not None:
                try:
                    ...
                    if backup_ai == "claude" and claude_client is not None:
                        try:
                            print(
                                f"Trying Claude backup for games: {game_names}")
                            ...
                            print(
                                f"Claude backup AI enhancement error: {claude_e}")
                ...
                try:
                    ...
                    if backup_ai == "gemini" and gemini_model is not None:
                        try:
                            print(
                                f"Trying Gemini backup for games: {game_names}")
                            response = gemini_model.generate_content(
                                prompt)  # type: ignore
                            if response and hasattr(
                                    response, 'text') and response.text:
                            ...
                            print(
                                f"Gemini backup AI enhancement error: {gemini_e}")

            if response_text:
                print(f"Raw AI response: {response_text[:200]}...")

                # Clean up response text - remove markdown code blocks if
                # present
                if response_text.startswith('```json'):
                    response_text = response_text[7:]
                if response_text.startswith('```'):
                ...
                try:
                    ...
                    print(
                        f"Successfully parsed JSON with {len(ai_data)} games")
                    ...
                    # Try to extract JSON from response if it's embedded in
                    # text
                    import re
...
async def refresh_ongoing_games_metadata() -> int:
    ...
    try:
        ...
        all_games = db.get_all_played_games()
        ongoing_games = [game for game in all_games if game.get(
                    ...
                    async with session.get(playlist_url_api, params=params) as response:
                        if response.status == 200:
                            ...
                            if data.get("items"):
                                ...
                                if current_video_count != game.get(
                                        'total_episodes', 0):
        ...
        if len(field_value) > 1024:
            ...
            for line in game_lines:
                if current_length + len(line) + 1 > 1000:  # Leave buffer
                    # Add current field - use empty string for field name to
                    # eliminate gaps
                    embed.add_field(
...
async def list_games(ctx):
    ...
    # Also update the persistent recommendations list in the recommendations
    # channel
    RECOMMEND_CHANNEL_ID = 1271568447108550687
    recommend_channel = ctx.guild.get_channel(RECOMMEND_CHANNEL_ID)
    if recommend_channel and ctx.channel.id != RECOMMEND_CHANNEL_ID:
        # Only update if we're not already in the recommendations channel to
        # avoid redundancy
        await post_or_update_recommend_list(ctx, recommend_channel)
...
async def remove_game(ctx, *, arg: str):
    ...
    RECOMMEND_CHANNEL_ID = 1271568447108550687
    recommend_channel = ctx.guild.get_channel(RECOMMEND_CHANNEL_ID)
    # Only send the detailed removal message in the invoking channel if not
        ...
        if not series_filter:
            ...
            for series, series_games in series_groups.items():
                ...
                if len(series_games) > 10:
                    game_lines.append(
                        f"... and {len(series_games) - 10} more games")
            ...
            game_lines = []
            for i, game in enumerate(
                    games[:20], 1):  # Limit to 20 for detailed view
                status_emoji = {
                    "completed": "✅",
                    "ongoing": "🔄",
                    "dropped": "❌",
                ...
                year = f" ({game.get('release_year')})" if game.get(
                    "release_year") else ""
                platform = f" [{game.get('platform')}]" if game.get(
                    "platform") else ""
            ...
            embed.add_field(name="Games List", value="\n".join(
                game_lines) if game_lines else "No games found", inline=False)
...
async def search_played_games_cmd(ctx, *, query: str):
    ...
    try:
        ...
        for i, game in enumerate(games[:15], 1):  # Limit to 15 results
            ...
            series = f" [{game.get('series_name')}]" if game.get(
                "series_name") else ""
            episodes = f" ({game.get('total_episodes', 0)} eps)" if game.get(
                "total_episodes", 0) > 0 else ""
            year = f" ({game.get('release_year')})" if game.get(
        ...
        embed.set_footer(
            text=f"Search query: {query} | Requested by {ctx.author.name}")
        embed.timestamp = discord.utils.utcnow()
...
async def game_info_cmd(ctx, *, identifier: str):
    ...
    try:
        ...
        if game.get("release_year"):
            embed.add_field(
                name="📅 Release Year", value=str(
                    game["release_year"]), inline=True)
        ...
        if game.get("total_episodes", 0) > 0:
            embed.add_field(
                name="📺 Episodes", value=str(
                    game["total_episodes"]), inline=True)
        ...
        if game.get("twitch_vod_urls"):
            ...
            embed.add_field(
                name="🎮 Twitch VODs",
                value=f"{vod_count} VODs available",
                inline=True)
...
async def update_played_game_cmd(
    ...
    try:
        ...
        if not updates or updates.strip() == "":
            ...
            if not game.get("alternative_names") or len(
                    game.get("alternative_names", [])) == 0:
                needs_update = True
                missing_fields.append("alternative_names")
            if not game.get('series_name') or game.get(
                    'series_name', '').strip() == '':
                needs_update = True
            ...
            if enhanced_games and len(enhanced_games) > 0:
                ...
                if enhanced_game.get(
                        "genre") and enhanced_game["genre"] != game.get("genre"):
                    ai_update_data["genre"] = enhanced_game["genre"]
                if enhanced_game.get(
                        "series_name") and enhanced_game["series_name"] != game.get("series_name"):
                    ai_update_data["series_name"] = enhanced_game["series_name"]
                if enhanced_game.get(
                        "release_year") and enhanced_game["release_year"] != game.get("release_year"):
                ...
                if ai_update_data:
                    # Apply AI updates using the same bulk import method that
                    # works reliably
                    complete_game_data = {
                        ...
                        "completion_status": game.get(
                            "completion_status",
                            "unknown"),
                        "total_episodes": game.get(
                            "total_episodes",
                            0),
                        "total_playtime_minutes": game.get(
                            "total_playtime_minutes",
                            0),
                        "youtube_playlist_url": game.get("youtube_playlist_url"),
                        "twitch_vod_urls": game.get(
                            "twitch_vod_urls",
                            []),
                    ...
                    updated_count = db.bulk_import_played_games(
                        [complete_game_data])
                    ...
                    if updated_count > 0:
                        ...
                        if ai_update_data.get("alternative_names"):
                            alt_names = ", ".join(
                                ai_update_data["alternative_names"])
        ...
        update_data = {}
        parts = [part.strip()
                 for part in updates.split('|')] if updates else []
...
async def bulk_import_played_games_cmd(
    ...
    try:
        ...
        if not youtube_api_key and not (
            ...
            try:
                ...
                for twitch_game in twitch_games:
                    ...
                    for yt_game in all_games_data:
                        if yt_game["canonical_name"].lower(
...
@commands.has_permissions(manage_messages=True)
async def fix_canonical_name_cmd(
        ctx,
        current_name: str,
        *,
                ...
                if success:
                    ...
                    if updated_game:
                        alt_names = ", ".join(
                            updated_game.get(
                                'alternative_names',
                                [])) if updated_game.get('alternative_names') else "None"
...
@commands.has_permissions(manage_messages=True)
async def add_alternative_name_cmd(
        ctx,
        game_name: str,
        *,
    ...
    try:
        ...
        if alternative_name.lower() in [name.lower()
        ...
        success = db.update_played_game(
            game['id'], alternative_names=updated_alt_names)
...
@commands.has_permissions(manage_messages=True)
async def remove_alternative_name_cmd(
        ctx, game_name: str, *, alternative_name: str):
    ...
    try:
        ...
        if success:
            remaining_names = ", ".join(
                updated_alt_names) if updated_alt_names else "None"
            await ctx.send(
...
async def update_played_games_cmd(ctx):
    ...
    try:
        ...
        # Filter games that need updates (missing genre, alternative_names, or
        # series_name)
        ...
        try:
            ...
            if msg.content == "CONFIRM UPDATE":
                ...
                # Convert database games to the format expected by
                # enhance_games_with_ai
                games_data = []
                ...
                # Apply updates to database using the same method as bulk
                # import
                updated_count = 0

                # Convert enhanced games back to the format expected by
                ...
                for enhanced_game in enhanced_games:
                    ...
                    if original_game:
                        # Create a complete game record for bulk import (which
                        # handles upserts)
                        game_data = {
                            "canonical_name": enhanced_game["canonical_name"],
                            "alternative_names": enhanced_game.get(
                            ...
                            "series_name": enhanced_game.get(
                                "series_name",
                                original_game.get("series_name")),
                            "genre": enhanced_game.get(
                                "genre",
                                original_game.get("genre")),
                            "release_year": enhanced_game.get(
                                "release_year",
                                original_game.get("release_year")),
                            ...
                            "completion_status": original_game.get(
                                "completion_status",
                                "unknown"),
                            "total_episodes": original_game.get(
                                "total_episodes",
                                0),
                            ...
                            "twitch_vod_urls": original_game.get(
                                "twitch_vod_urls",
                                []),
                ...
                # Run deduplication to merge any duplicate games created during
                # import
...
@commands.has_permissions(manage_messages=True)
async def set_alternative_names_cmd(
        ctx, game_name: str, *, alternative_names: str):
    ...
    try:
        ...
        alt_names_list = [name.strip()
                          for name in alternative_names.split(",") if name.strip()]
        ...
        if conn:
            try:
                with conn.cursor() as cur:
                    ...
                    cur.execute(
                        "SELECT canonical_name FROM played_games WHERE id = %s", (game_id_int,))
                    result = cur.fetchone()
                    if result:
                    ...
                    # Test if we can perform a harmless update (set updated_at
                    # to current time)
                    cur.execute(
...
async def remind_command(ctx, *, reminder_request: str):
    ...
    try:
        ...
        # Determine delivery method - DM for direct messages, channel for guild
        # messages
...
async def list_reminders_command(ctx):
    ...
    try:
        ...
        embed.set_footer(
            text=f"Total active reminders: {len(reminders)} | Use !cancelreminder <number> to cancel")
        embed.timestamp = discord.utils.utcnow()
