...
def _import_database_manager():
    ...
    try:
        ...
        db = DatabaseManager()
        print(
            f"✅ Database imported successfully from Live directory: {live_dir}")
        return True
    ...
    for i in range(3):  # Try up to 3 levels up
        try:
            ...
            db = DatabaseManager()
            print(
                f"✅ Database imported successfully from parent directory: {parent_dir}")
            return True
        except ImportError:
            continue
...
try:
    ...
    if not success:
        print(
            "❌ Database initialization failed - reminder and database features may not work")
        DatabaseManager = None
        db = None
