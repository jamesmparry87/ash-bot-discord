...
async def generate_contextual_trivia() -> Optional[Dict[str, Any]]:
    db = _get_db()
    if not db:
        return None

    categories = [
        'clip_famous_last_words',
        'clip_vibe_check',
        'clip_cause_and_effect',
        'clip_community_leaderboard',
        'game_platform_loyalty',
...
REAL CLIP DATA:
Game: {clip['game_title']}
Character/Enemy: {clip['characters_involved']}
Trigger: {clip['trigger']}
Reaction: {clip['reaction']}

Create a "Cause and Effect" question that asks what Jonesy's reaction was to this specific trigger/character, OR what trigger caused this specific reaction.
Provide 3 believable but incorrect decoy options (e.g. other extreme reactions like "Threw the controller", "Froze in silence").
Return strictly as JSON: {{"question_text": "...", "correct_answer": "...", "decoy_1": "...", "decoy_2": "...", "decoy_3": "...", "explanation": "Brief explanation."}}"""
...
Played on Platform: {game['platform']}

Create a "Platform Loyalty" question asking which console/platform she inexplicably decided to stream this game on.
The correct answer must be: {game['platform']}
Provide 3 believable but incorrect decoy platforms (e.g. PC, PS5, Switch, Xbox).
Return strictly as JSON: {{"question_text": "...", "correct_answer": "...", "decoy_1": "...", "decoy_2": "...", "decoy_3": "...", "explanation": "Brief explanation."}}"""
...
Total Episodes: {game['total_episodes']}

Create a "Series Marathon" question about her dedication to this franchise/game.
Provide 3 believable but incorrect decoy options.
Return strictly as JSON: {{"question_text": "...", "correct_answer": "...", "decoy_1": "...", "decoy_2": "...", "decoy_3": "...", "explanation": "Brief explanation."}}"""
...
REAL GAME DATA:
Game: {game['canonical_name']}
Original Release Year: {game['release_year']}
Year Jonesy finally played it: {play_year}

Create a "Retro vs Modern" question highlighting the gap between when it came out and when she played it. (e.g. "Which retro classic did Jonesy finally play in {play_year}, a full X years after it was released?")
Provide 3 believable but incorrect decoy options.
Return strictly as JSON: {{"question_text": "...", "correct_answer": "...", "decoy_1": "...", "decoy_2": "...", "decoy_3": "...", "explanation": "Brief explanation."}}"""
