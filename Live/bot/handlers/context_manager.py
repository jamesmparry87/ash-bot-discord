...
class ConversationContext:
    ...
    def __init__(self, user_id: int, channel_id: int):
        ...
        self.message_history: List[Dict[str, Any]] = []  # Recent message context
    ...
    def update_game_context(self, game_name: str, query_type: Optional[str] = None):
    ...
    def is_expired(self, minutes: int = 30) -> bool:
        ...
        cutoff = datetime.now(ZoneInfo("Europe/London")) - timedelta(minutes=minutes)
        return self.last_activity < cutoff


def get_or_create_context(user_id: int, channel_id: int) -> ConversationContext:
    ...
    if user_id not in conversation_contexts[channel_id]:
        conversation_contexts[channel_id][user_id] = ConversationContext(user_id, channel_id)
...
def cleanup_expired_contexts():
    ...
    for channel_id, user_id in expired_contexts:
        ...
        print(f"Cleaned up expired context for user {user_id} in channel {channel_id}")


def resolve_context_references(content: str, context: ConversationContext) -> Tuple[str, Dict[str, Any]]:
    """
    ...
    for pattern, replacement in subject_patterns:
        if re.search(pattern, resolved_content, re.IGNORECASE):
            resolved_content = re.sub(pattern, 'jonesy', resolved_content, flags=re.IGNORECASE)
...
def detect_follow_up_intent(content: str, context: ConversationContext) -> Optional[Dict[str, Any]]:
    """
    ...
    for follow_up in follow_up_patterns:
        for pattern in follow_up['patterns']:
            if re.search(pattern, content_lower):
                ...
                if follow_up.get('needs_game') and not context.last_mentioned_game:
                    continue
                if follow_up.get('needs_series') and not context.last_series_mentioned:
                    continue
