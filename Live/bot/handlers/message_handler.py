        ...
        try:
            ...
            if user.id == JONESY_USER_ID:
                ...
                if isinstance(mod_channel, discord.TextChannel):
                    await mod_channel.send("🛡️ **Strike attempt blocked:** Cannot strike Captain Jonesy. She is the commanding officer.")
                continue
        ...
        if ai_enabled:
            ...
            if match:
                ...
                if not clip_lore:
                    ...
                    success = await parser.process_clip(clip_url, message)
                    if success:
                        clip_lore = db.trivia.get_clip_lore(canonical_url)
                        import asyncio
...
Respond to: {content}"""
            ...
            if response_text:
                ...
                print("⚠️ CRITICAL AI ERROR: AI call returned None")
            ...
            # ADD LOUD ERROR LOGGING
            print("⚠️ CRITICAL AI ERROR: AI is not enabled")
