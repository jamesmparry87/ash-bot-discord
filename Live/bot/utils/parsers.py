...
def parse_command_arguments(
        content: str, command_prefix: str = "!") -> Dict[str, Any]:
...
def parse_quoted_strings(text: str) -> List[str]:
    ...
    for match in re.finditer(quote_pattern, text):
        ...
        unquoted = quoted_text[1:-1].replace('\\"',
                                             '"').replace("\\'",
                                                          "'").replace('\\\\',
                                                                       '\\')
    ...
    for pattern in episode_patterns:
        ...
        if match:
            ...
            working_title = re.sub(
                pattern, '', working_title, flags=re.IGNORECASE).strip()
            break
...
def extract_code_blocks(text: str) -> List[Dict[str, str]]:
    ...
    for match in re.finditer(inline_pattern, text):
        ...
        if not any(
            match.start() >= block['full_match'].find(
                block['code']) and match.end() <= block['full_match'].find(
                block['code']) + len(
                block['code']) for block in code_blocks if block['type'] == 'multiline'):
            code_blocks.append({
                'type': 'inline',
                'language': '',
                'code': match.group(1),
