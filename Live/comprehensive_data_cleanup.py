...
def detect_dlc_suffix(game_name: str) -> bool:
    ...
    if re.search(r'\b(GOTY|Game of the Year|Complete|Definitive|Ultimate|Remastered)\s+Edition\b', game_name, re.IGNORECASE):
        return False
...
async def cleanup_alternative_names(db, dry_run: bool = True) -> Dict[str, Any]:
    ...
    for game in games:
        try:
            ...
            if isinstance(alt_names_raw, str) and ('{' in alt_names_raw or '"' in alt_names_raw or '\\' in alt_names_raw):
                # Try to parse
