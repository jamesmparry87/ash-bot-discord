...
async def add_god_of_war_1():
    ...
    if success:
        ...
        if new_entry:
            ...
            # If get_played_game returns the fuzzy match (GoW II), try to fetch the
            # new one manually or just trust the success bool
            if new_entry.get('canonical_name') != target_name:
                print(
                    f"   (Note: 'get_played_game' returned '{new_entry.get('canonical_name')}' due to fuzzy search, but insert was successful.)")
            else:
