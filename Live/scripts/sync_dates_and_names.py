...
def get_twitch_token():
    ...
    if _twitch_access_token: return _twitch_access_token
...
def get_twitch_date(vod_url_field):
    if not vod_url_field: return None
...
def get_youtube_date(playlist_url):
    if not playlist_url: return None
...
def clean_messy_string(text):
    ...
    if not items and cleaned: items = cleaned.split(',')
    return [item.replace('\\"', '').strip() for item in items if item.replace('\\"', '').strip() not in ['{', '}', '""']]
...
def process_data():
    ...
    for game in RAW_DATA:
        ...
        if name in SMART_MAPPINGS: current_alts.extend(SMART_MAPPINGS[name])
