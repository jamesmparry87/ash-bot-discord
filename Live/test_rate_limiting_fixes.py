...
def test_priority_intervals():
    ...
    try:
        ...
        assert PRIORITY_INTERVALS["high"] == 1.0, f"High priority should be 1.0s, got {PRIORITY_INTERVALS['high']}"
        assert PRIORITY_INTERVALS["medium"] == 2.0, f"Medium priority should be 2.0s, got {PRIORITY_INTERVALS['medium']}"
        assert PRIORITY_INTERVALS["low"] == 3.0, f"Low priority should be 3.0s, got {PRIORITY_INTERVALS['low']}"
        
        print("✅ Priority intervals configured correctly")
        
        # Check progressive cooldowns
        assert RATE_LIMIT_COOLDOWNS["first"] == 30, f"First cooldown should be 30s, got {RATE_LIMIT_COOLDOWNS['first']}"
        assert RATE_LIMIT_COOLDOWNS["second"] == 60, f"Second cooldown should be 60s, got {RATE_LIMIT_COOLDOWNS['second']}"
        assert RATE_LIMIT_COOLDOWNS["third"] == 120, f"Third cooldown should be 120s, got {RATE_LIMIT_COOLDOWNS['third']}"
        assert RATE_LIMIT_COOLDOWNS["persistent"] == 300, f"Persistent cooldown should be 300s, got {RATE_LIMIT_COOLDOWNS['persistent']}"
...
def test_priority_determination():
    ...
    try:
        ...
        assert determine_request_priority("What is the answer?", 123, "trivia") == "high", "Trivia context should be high priority"
        assert determine_request_priority("What is Captain Jonesy's favorite game?", 123) == "high", "Direct questions should be high priority"
        
        # Test medium priority cases  
        assert determine_request_priority("Hello there", 123) == "medium", "Greetings should be medium priority"
        assert determine_request_priority("Can you help me?", 123) == "medium", "Help requests should be medium priority"
        
        # Test low priority cases
        background_result = determine_request_priority("Rewrite this announcement", 123, "background")
        ...
        announcement_result = determine_request_priority("This is an announcement to rewrite", 123)
...
def test_rate_limit_functions():
    ...
    try:
        ...
        assert get_progressive_penalty_duration(0) == 0, "No penalty for 0 errors"
        assert get_progressive_penalty_duration(2) == 0, "No penalty for 2 errors"
        assert get_progressive_penalty_duration(3) == 30, "30s penalty for 3 errors"
        assert get_progressive_penalty_duration(4) == 60, "60s penalty for 4 errors"
        assert get_progressive_penalty_duration(5) == 120, "120s penalty for 5 errors"
        assert get_progressive_penalty_duration(10) == 300, "300s penalty for 10+ errors"
        ...
        print(f"✅ Rate limit check function callable: can_request={can_request}, reason='{reason}'")
