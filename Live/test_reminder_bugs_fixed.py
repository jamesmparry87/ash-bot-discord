...
def test_dot_time_parsing():
    ...
    try:
        ...
        assert result['reminder_text'].strip() == "stand up", f"Should extract 'stand up', got '{result['reminder_text']}'"
        ...
        for test_input, expected_text, expected_hour, expected_minute in test_cases:
            ...
            print(f"    Time: {result['scheduled_time'].strftime('%H:%M')} (expected: {expected_hour:02d}:{expected_minute:02d})")
