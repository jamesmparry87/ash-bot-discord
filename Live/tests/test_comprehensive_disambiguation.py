        ...
        for faq in required_faqs:
            if faq in FAQ_RESPONSES:
                ...
                if faq == "who is jonesy":
                    if "Captain Jonesy" in response and any(
                        phrase in response for phrase in [
                            "server", "commanding officer", "Discord"]):
...
def test_ai_prompt_fixes():
    ...
    try:
        ...
        conversation_handler_count = len(
            re.findall(
                disambiguation_rule_pattern,
                conversation_handler_content,
        ...
        if conversation_handler_count >= 2:
            ...
            print(
                f"  ⚠️ Conversation handler has disambiguation rule in {conversation_handler_count} places (expected 2+)")
