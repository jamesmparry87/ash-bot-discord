...
def test_question_generation_end_to_end():
    ...
    try:
        ...
        if len(generated_questions) >= 3:
            ...
            playtime_questions = [q for q in generated_questions if 'playtime' in q['question'].lower() or
                                  'longest' in q['question'].lower()]

            if len(playtime_questions) < len(generated_questions) / 2:
                print(
                    f"✅ Only {len(playtime_questions)}/{len(generated_questions)} questions are about playtime - good diversity!")
                return True
            else:
