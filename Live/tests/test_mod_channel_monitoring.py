...
def run_tests():
    ...
    print(f"  Mod channels blocked:    {mod_background_blocked}/{len(background_monitoring_cases)} {'✅' if mod_background_blocked == len(background_monitoring_cases) else '❌'}")
    print(f"  Normal channels allowed: {normal_background_allowed}/{len(background_monitoring_cases)} {'✅' if normal_background_allowed == len(background_monitoring_cases) else '❌'}")
    print()
    print(f"Direct Interactions (should work everywhere):")
    print(f"  Mod channels allowed:    {mod_direct_allowed}/{len(direct_interaction_cases)} {'✅' if mod_direct_allowed == len(direct_interaction_cases) else '❌'}")
    print(f"  Normal channels allowed: {normal_direct_allowed}/{len(direct_interaction_cases)} {'✅' if normal_direct_allowed == len(direct_interaction_cases) else '❌'}")
    print()
    print(f"Ignored Cases (should be ignored everywhere):")
    print(f"  Mod channels ignored:    {mod_ignored}/{len(ignored_cases)} {'✅' if mod_ignored == len(ignored_cases) else '❌'}")
    print(f"  Normal channels ignored: {normal_ignored}/{len(ignored_cases)} {'✅' if normal_ignored == len(ignored_cases) else '❌'}")
    print()
